function [n_patterns, ...
    ...
    mean_steps, ...
    distortPerce] = ...
    MNISTPatterns (data_dir, n_patterns, n_trials, distortPerce, showOff)
...
for reps = start_pattern:n_patterns
    ...
    for trial = 1:n_trials
        ...
        noise = randperm(N,round(N*distortPerce));
        randPattern(noise) = randPattern(noise) * -1;
        ...
        while existPattern
            ...
            if showOff
                ...
                axis off;
                title({['MNIST ', num2str(distortPerce*100),'% distorted pattern']; [num2str(steps), ' steps']}, 'FontSize', 18);
                hold off
...
if ~showOff
    ...
    set(h1, 'Name', 'P(error) ~ Patterns - MNIST', 'NumberTitle', 'off');
    hold on;
    ...
    title({'MNIST recall error probability'; 'as a function of number of patterns'}, 'FontSize', 18);
    hold off;
    
    h2 = figure('units','normalized', 'Position', [0.5 0.2 0.5 0.5]);
    set(h2, 'Name', 'Mean time steps ~ Patterns - MNIST', 'NumberTitle', 'off');
    hold on;
    ...
    title({'MNIST average time steps until recall'; 'as a function of number of patterns'}, 'FontSize', 18);
    hold off;
end
