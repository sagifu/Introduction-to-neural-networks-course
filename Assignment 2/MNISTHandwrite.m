function [n_patterns, ...
    P_error, ...
    mean_steps] = ...
    MNISTHandwrite (data_dir, n_patterns, n_trials, showOff)
...
for reps = start_pattern:n_patterns
    ...
    for trial = 1:n_trials
        ...
        randPatternOrig = memory(:,randP);
        sameNumber = train_labels(select_num(randP));
        
        numberLocation = randi(length(test_labels));
        number = test_labels(numberLocation);
        ...
        randPattern = preprocess(test_images(:,:,numberLocation), 'yes');
        ...
        while existPattern
            ...
            if showOff
                ...
                axis off;
                title({['MNIST different hand write pattern']; [num2str(steps), ' steps']}, 'FontSize', 18);
                hold off
...
if ~showOff
    ...
    set(h1, 'Name', 'P(error) ~ Patterns - MNIST - differnet hand writing', 'NumberTitle', 'off');
    hold on;
    ...
    title('MNIST different hand writing recall error probability as a function of number of patterns');
    hold off;
    
    h2 = figure('units', 'normalized', 'Position', [0.5 0.2 0.5 0.5]);
    set(h2, 'Name', 'Mean time steps ~ Patterns - MNIST - differnet hand writing', 'NumberTitle', 'off');
    hold on;
    ...
    title('MNIST different hand writing average time steps until recall as a function of number of patterns');
    hold off;
end
